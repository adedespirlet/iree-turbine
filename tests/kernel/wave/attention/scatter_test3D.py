...
def test_read_actual_data():
    ...
    def read_kernel(
        a: tkl.Memory[B, M, N, GLOBAL_ADDRESS_SPACE, tkl.i32],
        index: tkl.Memory[B, M, N, GLOBAL_ADDRESS_SPACE, tkl.i32],
        lds: tkl.Memory[B, M, N, ADDRESS_SPACE, tkl.i32],
        b: tkl.Memory[B, M, N, GLOBAL_ADDRESS_SPACE, tkl.i32],
    ):
    ...
    def scatter_baseline(input, index):
        ...
        baseline_output = torch.zeros_like(input)
        return baseline_output.scatter_add(dim=1, index=index, src=input)

    torch_output = scatter_baseline(input, index)
    print("torch_output:")
    print(torch_output)
    torch.testing.assert_close(output, torch_output)
    print("Test passed! 3D scatter_add matches baseline.")
